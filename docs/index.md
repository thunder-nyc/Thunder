...
Thunder is a [C++11](http://en.wikipedia.org/wiki/C%2B%2B11) library to provide device-transparent Tensor mathematical operations. The current development is focusing on applications in [Deep Learning](http://en.wikipedia.org/wiki/Deep_learning) using high-level parallelization and numerical optimization with GPUs and computer clusters.
...
* A Fortran compiler, such as the gfortran compiler included in [gcc](https://gcc.gnu.org).
* BLAS library
...
Thunder has many exciting features. The following is a preview list. Most of them are already in the current public source code.
